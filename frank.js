...
client.on('message', async message => {
  ...
  //Play tts
client.on('message', async message => {
  if (message.channel.id === "840470180084711444") {
    if (message.member.voice.channel) {
      const connection = await message.member.voice.channel.join();
      await quickStart(String(message.content));
      const dispatcher = connection.play('out.mp3');
    } else {
      message.reply('You need to join a voice channel first!');
    }
  }
...
client.on('message', async message => {
  ...
  if(message.content.includes("<@!589153934392950804>")) {
    
    
    console.log(bad_words.some(word => message.content.includes(word)));
...
client.on('message', async message => {
  if(message.content.includes("genshin") || message.content.includes("Genshin")) {
    message.channel.send("Your game is trash")
  }
})

client.on('message', async message => {
  if(message.channel.id === "747660053531590706" && message.author.bot === false) {
    let ttsmessage = message.content
    
    
  }
})
async function quickStart(messages) {
  ...
  const request = {
    ...
    voice: {languageCode: 'en-US', ssmlGender: 'NEUTRAL', voiceName: "en-US-Standard-D"},
...
function gotMessage(msg) {
    ...
    msg.channel.id === '688450856831811591') {
        if(msg.author.userwname === 'tobyteatime' || 
